...
조회된 기간의 런처 실행 수 와런처 실행 실패 이유의 차트데이터를 제공 합니다.
...
> 4-1) `Game Update` : 게임 업데이트 시도한 수
> 4-1) `Game Update Bounce` : 게임 업데이트 중 이탈한 수
> 4-1) `Game Execution` : 게임 업데이트나 설치를 시도한 유저중 실행까지 시도한 
>